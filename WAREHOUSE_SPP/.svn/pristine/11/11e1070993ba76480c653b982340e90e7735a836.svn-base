...
//
    ...
    /// </summary>
    public partial class frm_lap_phieu_xuat_kho_vattu_dich_vu : ControlBase
    {
        ...
        IKho_DuyetPhieuYeuCau_DVBussiness _DPYC;
        DataRow iRowSelGb = null;
        DataRow iRowSelGb_CT = null;
        int iD_PhieuYC = 0;
        public frm_lap_phieu_xuat_kho_vattu_dich_vu()
        {
            InitializeComponent();
            _YCDV = new Kho_Phieu_YeuCau_DVBussiness(string.Empty);
            _DPYC = new Kho_DuyetPhieuYeuCau_DVBussiness(string.Empty);
            Initialize_Grid();
            Initialize_Grid_CT();
        ...
        private void Initialize_Grid()
        {
            try
            {
                ...
                xColumn = new Column("TEN_TRANGTHAI_DUYET", "Trạng thái duyệt");
                xColumn.Width = 100;
                xColumn.Visible = true;
                xColumn.HorizontalContentAlignment = DevExpress.Xpf.Editors.Settings.EditSettingsHorizontalAlignment.Left;
                xColumn.AllowEditing = DevExpress.Utils.DefaultBoolean.False;
        ...
        private void grdView_MouseDoubleClick(object sender, MouseButtonEventArgs e)
        {
            try
            {
                ...
                DataRow[] iRowSelGb_NVL = _DPYC.GetList_LamPhieuDV_CTiet_NVL(CommonDataHelper.DonViQuanLy, iD_PhieuYC).Select("PHIEUYEUCAU_DV_ID=" + iD_PhieuYC);
                if(iRowSelGb["TRANGTHAI_DUYET"].ToString() == "1")
                {
                    object xReturn = UIAgent.GetUIPopupDialog(this, null, this.GetType().ToString(), "DSofT.Warehouse.UI", "frm_lap_phieu_xuat_kho_vattu_dich_vu_popup", iRowSelGb, iRowSelGb_ctiet, iRowSelGb_NVL);
                    LoadDataGrdDichVu();
                }
                else
                {
                    base.ShowMessage(MessageType.Information, "VUI LÒNG CHỌN PHIẾU ĐÃ ĐƯỢC DUYỆT");
                    return;
                }
