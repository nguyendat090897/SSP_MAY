...
namespace DSofT.Warehouse.UI
{
    ...
    ///  Editor:      
    ///  Modify date: 
    ///  Description: Danh muc quoc gia
    /// </summary>
    public partial class frmDanhMucQuocGia : ControlBase
    {
        ...
        private void Initialize_Grid()
        {
            try
            {
                ...
                xColumn = new Column("MA_QUOCGIA", Convert.ToString(UtilLanguage.ApplyLanguage()["frmDMQG_Ma"]));
                xColumn.Width = 100;
                xColumn.Visible = true;
                ...
                xColumn = new Column("TEN_QUOCGIA", Convert.ToString(UtilLanguage.ApplyLanguage()["frmDMQG_Ten"]));
                xColumn.Width = 400;
                xColumn.Visible = true;
                xColumn.HorizontalContentAlignment = DevExpress.Xpf.Editors.Settings.EditSettingsHorizontalAlignment.Left;
                xColumn.AllowEditing = DevExpress.Utils.DefaultBoolean.False;
                ...
                xColumn = new Column("TEN_VIETTAT", Convert.ToString(UtilLanguage.ApplyLanguage()["frmDMQG_TenVT"]));
                xColumn.Width = 300;
                xColumn.Visible = true;
                xColumn.HorizontalContentAlignment = DevExpress.Xpf.Editors.Settings.EditSettingsHorizontalAlignment.Left;
                xColumn.AllowEditing = DevExpress.Utils.DefaultBoolean.False;
        ...
        private void btnNew_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                object xReturn = UIAgent.GetUIPopupDialog(this, null, this.GetType().ToString(), "DSofT.Warehouse.UI", "frmDanhMucQuocGia_Popup", null);
                LoadData();
            }
            catch (Exception ex)
            {
        ...
        private void LoadData()
        {
            try
            {
                using (var task = new AsyncTaskProcess(this))
                {
                    ...
                    {
                        task.SetCallContext();
                        return _DM_QUOCGIABussiness.GetListDM_QUOCGIA(ConstCommon.DonViQuanLy);
                    });
        ...
        private void grdView_MouseDoubleClick(object sender, MouseButtonEventArgs e)
        {
            ...
            try
            {
                ...
                object xReturn = UIAgent.GetUIPopupDialog(this, null, this.GetType().ToString(), "DSofT.Warehouse.UI", "frmDanhMucQuocGia_Popup", iRowSelGb);
                LoadData();
            }
            catch (Exception ex)
            {
        ...
        private void btnDelete_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                bool result = _DM_QUOCGIABussiness.DeleteDM_QUOCGIA(ConstCommon.DonViQuanLy, Convert.ToInt32(iRowSelGb["QUOCGIA_ID"].ToString()), CommonDataHelper.UserName);
                if (!result)
                {
