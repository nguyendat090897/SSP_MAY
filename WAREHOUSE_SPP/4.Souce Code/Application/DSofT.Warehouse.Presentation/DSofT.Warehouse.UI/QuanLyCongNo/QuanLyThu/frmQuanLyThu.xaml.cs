    ...
    public partial class frmQuanLyThu : ControlBase
    {
        ...
        private void Initialize_Grid()
        {
            try
            {
                ...
                xColumn = new Column("SO_PHIEUTHU", Convert.ToString(UtilLanguage.ApplyLanguage()["lblSoPhieuThu"]));
                xColumn.Width = 100;
                xColumn.Visible = true;
        ...
        void LoadData()
        {
            try
            {
                using (var task = new AsyncTaskProcess(this))
                {
                    ...
                    {
                        task.SetCallContext();
                        return _QuanLy_PhieuThu_KH_NCC_Bussiness.GetAll(ConstCommon.DonViQuanLy,"THU");
                    });
        ...
        private void grdView_MouseDoubleClick(object sender, MouseButtonEventArgs e)
        {
            try
            {
                ...
                object xReturn = UIAgent.GetUIPopupDialog(this, null, this.GetType().ToString(), "DSofT.Warehouse.UI", "frmQuanLyThu_Popup", iRowSelGb);
                LoadData();
            }
            catch (Exception ex)
            {
