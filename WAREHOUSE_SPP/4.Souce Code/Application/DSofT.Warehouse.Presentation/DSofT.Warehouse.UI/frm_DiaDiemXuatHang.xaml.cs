...
{
    /// <summary>
    /// Interaction logic for frm_DiaDiemXuatHang.xaml
    /// </summary>
    public partial class frm_DiaDiemXuatHang : PopupBase
    {
        ...
        private void LoadData()
        {
            try
            {
                ...
                using (var task = new AsyncTaskProcess(this))
                {
                    ...
                    {
                        task.SetCallContext();
                        return _DM_KHACHHANG_NCCBussiness.GetListDM_KHACHHANG_NCC_DIA_DIEM_XUAT_HANG(ConstCommon.DonViQuanLy, frm_dm_khachhang_ncc_popup.pKHACHHANG_NCC_ID);
                    });
        ...
        private void Initialize_Grid()
        {
            try
            {
                ...
                xColumn = new Column("DIADIEM_GIAO", Convert.ToString(UtilLanguage.ApplyLanguage()["lblDiaDiemGiao"]));
                xColumn.Width = 150;
                xColumn.MaxLenth = 50;
                xColumn.Visible = true;
        ...
        void xButton_RemoveClick(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (ConstCommon.NVL_NUM_LONG_NEW(row.Row["KHACHHANG_NCC_NOI_XUATHANG_ID"]) > 0)
                {
                    if (this.iGridDataSource != null && this.iGridDataSource.Rows[(grd.View as TableView).FocusedRowHandle]["KHACHHANG_NCC_NOI_XUATHANG_ID"].ToString() != string.Empty)
                    {
                        if (!base.ShowMessage(MessageType.OkCancel, "Bạn muốn xóa không?"))
                            return;
                        bool result = _DM_KHACHHANG_NCCBussiness.DeleteDM_KHACHHANG_NCC_DIA_DIEM_XUAT_HANG(ConstCommon.DonViQuanLy, ConstCommon.NVL_NUM_LONG_NEW(this.iGridDataSource.Rows[(grd.View as TableView).FocusedRowHandle]["KHACHHANG_NCC_NOI_XUATHANG_ID"].ToString()), CommonDataHelper.UserName);
                        if (!result)
                        {
        ...
        private void btnSave_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (newrow == true)
                {
                    ...
                    {
                        if (this.iGridDataSource.Rows[i]["DIADIEM_GIAO"].ToString() == String.Empty || this.iGridDataSource.Rows[i]["TINH_TRANG"].ToString() == String.Empty)
                        {
                            flag = false;
                            if (this.iGridDataSource.Rows[i]["DIADIEM_GIAO"].ToString() == String.Empty)
                            {
                                note = "[ĐỊA ĐIỂM GIAO] ";
                                break;
                            }
                ...
                if (flag == true)
                {
                    ...
                    {
                        DataTable dt = new DataTable();
                        dt.Columns.Add("KHACHHANG_NCC_NOI_XUATHANG_ID");
                        dt.Columns.Add("STT");
                        dt.Columns.Add("DIADIEM_GIAO");
                        dt.Columns.Add("DIA_CHI");
                        dt.Columns.Add("GHI_CHU");
                        dt.Columns.Add("TINH_TRANG");
                        if (this.iGridDataSource.Rows[i]["DIADIEM_GIAO"].ToString() != String.Empty)
                        {
                            dt.Rows.Add(this.iGridDataSource.Rows[i]["KHACHHANG_NCC_NOI_XUATHANG_ID"].ToString(), (i + 1),
                                this.iGridDataSource.Rows[i]["DIADIEM_GIAO"].ToString(), this.iGridDataSource.Rows[i]["DIA_CHI"].ToString(),
                                this.iGridDataSource.Rows[i]["GHI_CHU"].ToString(), this.iGridDataSource.Rows[i]["TINH_TRANG"].ToString());
                            result = _DM_KHACHHANG_NCCBussiness.InsertorUpdateDIA_DIEM_XUAT_HANG(dt, CommonDataHelper.UserName, frm_dm_khachhang_ncc_popup.pKHACHHANG_NCC_ID);
                        }
                    }
                    if (!result)
                    {
                    ...
                    {
                        if (this.iGridDataSource.Rows[0]["DIADIEM_GIAO"].ToString() == String.Empty && this.iGridDataSource.Rows[0]["GHI_CHU"].ToString() == string.Empty && this.iGridDataSource.Rows[0]["TINH_TRANG"].ToString() == String.Empty)
                            return;
                        if (this.iGridDataSource.Rows[0]["DIADIEM_GIAO"].ToString() == String.Empty && this.iGridDataSource.Rows[0]["TINH_TRANG"].ToString() == String.Empty && this.iGridDataSource.Rows[0]["GHI_CHU"].ToString() != string.Empty)
                            base.ShowMessage(MessageType.Information, "[ĐỊA ĐIỂM GIAO],[TÌNH TRẠNG HOẠT ĐỘNG] chưa chọn.");
                        else
                        {
            ...
            try
            {
                if (this.iGridDataSource != null)
                {
                    if ((grd.View as TableView).FocusedRowHandle == this.iGridDataSource.Rows.Count - 1)
                    {
                        ...
                        if (grd.GetFocusedRowCellValue("DIADIEM_GIAO").ToString() != string.Empty && grd.GetFocusedRowCellValue("TINH_TRANG").ToString() != string.Empty)
                        {
                            this.iGridDataSource.Rows.Add();
                            newrow = true;
                        }
