...
namespace DSofT.Warehouse.UI
{
    ...
    /// </summary>
    public partial class frm_XuatKho : ControlBase
    {
        ...
        public void LoadData()
        {
            try
            {
                using (var task = new AsyncTaskProcess(this))
                {
                    ...
                    {
                        task.SetCallContext();
                        return _IPHIEU_NHAP_XUAT_KHO.getKO_PHIEU_NHAPXUATKHO_ALL(ConstCommon.DonViQuanLy, "X");
                    });
        ...
        private void btnNew_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                frm_NhapKho_popup.status = false;
                frm_NhapKho_popup.pN_OR_N = "X";
                object xReturn = UIAgent.GetUIPopupDialog(this, null, this.GetType().ToString(), "DSofT.Warehouse.UI", "frm_NhapKho_popup"/*,null,"X"*/);
                LoadData();
            }
            catch (Exception ex)
            {
        ...
        private void grdViewLapPhieuNhapKho_MouseDoubleClick(object sender, MouseButtonEventArgs e)
        {
            try
            {
                ...
                xReturn = UIAgent.GetUIPopupDialog(this, null, this.GetType().ToString(), "DSofT.Warehouse.UI", "frm_NhapKho_popup", iRowSelGb, iRowSelGb_ctiet/*, "X",pLA_KIEM_KIEM*/);
                LoadData();
            }
            catch (Exception ex)
            {
        ...
        private void btnXepHangVaoKho_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                DataRow iRowSelGb = ((DataRowView)this.gridControl.GetFocusedRow()).Row;
                DataRow[] iRowSelGb_ctiet = iGridDataSource_CTPHIEUNHAP.Tables[1].Select("PHIEU_NHAPXUATKHO_ID=" + ConstCommon.NVL_NUM_LONG_NEW(iRowSelGb["PHIEU_NHAPXUATKHO_ID"]));
                xReturn = UIAgent.GetUIPopupDialog(this, null, this.GetType().ToString(), "DSofT.Warehouse.UI", "frm_NhapKho_popup_xep_hang_vao_kho", iRowSelGb, iRowSelGb_ctiet, "X");
                LoadData();
            }
            catch (Exception ex)
            {
