...
{/// //
    /// <summary>
    /// </summary>
    public partial class frm_dm_LoaiSanPham : ControlBase
    {
        DataRow iRowSelGb = null;
        DataTable iDataSource = null;
        GridHelper iGridHelper = null;
        DataTable iGridDataSource = null;
        IDM_LOAI_SPHAMBussiness _DM_LOAI_SPHAMBussiness;
        public static bool status = true;
        public frm_dm_LoaiSanPham()
        {
            _DM_LOAI_SPHAMBussiness = new DM_LOAI_SPHAMBussiness(string.Empty);
            InitializeComponent();
            Initialize_Grid(); 
        ...
        private void Initialize_Grid()
        {
            try
            {
                ...
                xColumn = new Column("MA_LOAI_SPHAM", Convert.ToString(UtilLanguage.ApplyLanguage()["lblMaLoaiSanPham"]));
                xColumn.Width = 80;
                xColumn.Visible = true;
                ...
                xColumn = new Column("TEN_LOAI_SPHAM", Convert.ToString(UtilLanguage.ApplyLanguage()["lblTenLoaiSanPham"]));
                xColumn.Width = 100;
                xColumn.Visible = true;
                xColumn.HorizontalContentAlignment = DevExpress.Xpf.Editors.Settings.EditSettingsHorizontalAlignment.Left;
                xColumn.AllowEditing = DevExpress.Utils.DefaultBoolean.False;
        ...
        private void btnNew_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                object xReturn = UIAgent.GetUIPopupDialog(this, null, this.GetType().ToString(), "DSofT.Warehouse.UI", "frm_dm_LoaiSanPham_popup", null);
                LoadData();
            }
            catch (Exception ex)
            {
        ...
        private void LoadData()
        {
            try
            {
                using (var task = new AsyncTaskProcess(this))
                {
                    ...
                    {
                        task.SetCallContext();
                        return _DM_LOAI_SPHAMBussiness.GetListDM_LOAI_SPHAM(ConstCommon.DonViQuanLy);
                    });
        ...
        private void grdView_MouseDoubleClick(object sender, MouseButtonEventArgs e)
        {
            ...
            try
            {
                ...
                object xReturn = UIAgent.GetUIPopupDialog(this, null, this.GetType().ToString(), "DSofT.Warehouse.UI", "frm_dm_LoaiSanPham_popup", iRowSelGb);
                LoadData();
            }
            catch (Exception ex)
            {
        ...
        private void btnDelete_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                bool result = _DM_LOAI_SPHAMBussiness.DeleteDM_LOAI_SPHAM(ConstCommon.DonViQuanLy, Convert.ToInt32(iRowSelGb["LOAI_SPHAM_ID"].ToString()), CommonDataHelper.UserName);
                if (!result)
                {
