    ...
    public partial class frm_lap_yeu_cau_dich_vu : ControlBase
    {
        ...
        private void grdView_MouseDoubleClick(object sender, MouseButtonEventArgs e)
        {
            try
            {
                ...
                DataRow[] iRowSelGb_NVL= _YCDV.GetList_LamPhieuDV_CTiet_NVL(CommonDataHelper.DonViQuanLy, iD_PhieuYC).Select("PHIEUYEUCAU_DV_ID=" + iD_PhieuYC);

                object xReturn = UIAgent.GetUIPopupDialog(this, null, this.GetType().ToString(), "DSofT.Warehouse.UI", "frm_lap_yeu_cau_dich_vu_popup", iRowSelGb, iRowSelGb_ctiet, iRowSelGb_NVL);
                LoadDataGrdDichVu();
            }
            catch (Exception ex)
            {
