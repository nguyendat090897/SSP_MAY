    ...
    public partial class frm_lapphieu_yeucau_nhapkho_popup : PopupBase
    {
        ...
        private void BuildCombobox()
        {
            try
            {
                ...
                DataTable dtHinhhthucnhap = Yeucauxuatkho_bus.getDM_HINHTHU_NHAPXUAT(pN_OR_N);
        ...
        private DataTable TableSchemaDefineBingding_Grid()
        {
            ...
            try
            {
                ...
                xDicUser.Add("MA_DVIQLY", typeof(string));
                xDicUser.Add("PHIEUYEUCAU_NHAPXUATKHO_CTIET_ID", typeof(decimal));
                xDicUser.Add("PHIEUYEUCAU_NHAPXUATKHO_ID", typeof(decimal));
                    ...
                    if (dt_SPCHON.Rows[0]["GHI_CHU"].ToString() == "Y")
                    {
                        ...
                        for (int i = 0; i < dt_SPCHON.Rows.Count; i++)
                        {
                            if (ContainDataRowInDataTable(dt_PHIEU_CTIET, dt_SPCHON.Rows[i]) == false)
                            {
                                ...
                                dr["PHIEUYEUCAU_NHAPXUATKHO_ID"] = "0";
        ...
        private void btnXoadong_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (ConstCommon.NVL_NUM_LONG_NEW(row.Row["PHIEUYEUCAU_NHAPXUATKHO_CTIET_ID"]) > 0)
                {
                    msg = (bool)base.ShowMessage(MessageType.OkCancel, "Bạn có muốn xóa dòng đang chọn trong phiếu không");
                    if (msg == true)
                    {
                        DataTable dsReturn = Yeucauxuatkho_bus.KO_PHIEUYEUCAU_NHAPXUATKHO_CTIET_DELETE(ConstCommon.DonViQuanLy, ConstCommon.NVL_NUM_LONG_NEW(row.Row["PHIEUYEUCAU_NHAPXUATKHO_CTIET_ID"]), CommonDataHelper.UserName);
        ...
        private bool ValidUI()
        {
            try
            {
                ...
                {
                    base.ShowMessage(MessageType.Information, "Vui lòng không nhập ký tự đặc biệt Số phiếu ");
                    txtSOPHIEU.Focus();
                    return false;
                }
                ...
                {
                    base.ShowMessage(MessageType.Information, "Vui lòng không nhập ký tự đặc biệt Số Conts ");
                    txtSOPHIEU.Focus();
                    return false;
                }
                ...
                if (pN_OR_N == "N")
                {
                    ...
                    {
                        base.ShowMessage(MessageType.Information, "Vui lòng nhập Ngày nhập ");
                        txtNGAYLAP.Focus();
                        return false;
                    }
                ...
                else
                {
                    if (txtNGAYLAP.Text.Trim() == "")
                    {
                        base.ShowMessage(MessageType.Information, "Vui lòng nhập Ngày xuất ");
                        txtNGAYLAP.Focus();
                        return false;
                    }
                    ...
                    if (cbohinhthucnhap.Text == "--Chọn--")
                    {
                        base.ShowMessage(MessageType.Information, "Vui lòng chọn hình thức xuất ");
                        cbohinhthucnhap.Focus();
                        return false;
                    }
                ...
                if (Yeucauxuatkho_bus.KO_PHIEUYEUCAU_NHAPXUATKHO_CHECKEXISTS(ConstCommon.DonViQuanLy, ConstCommon.NVL_NUM_LONG_NEW(txtPHIEUYEUCAU_NHAPKHO_ID.Text), txtSOPHIEU.Text.Trim()) == true)
                {
        ...
        private void btnSave_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                dsReturn = Yeucauxuatkho_bus.InsertKO_PHIEUYEUCAU_NHAPXUATKHO(iDataSource, dt_PHIEU_CTIET, CommonDataHelper.UserName, pN_OR_N);
