...
{
    /// <summary>
    /// Interaction logic for popupWarehouse.xaml
    /// </summary>
    public partial class popupWarehouse : PopupBase
    {
        ...
        private void btnDelete_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                if (!base.ShowMessage(MessageType.OkCancel, Convert.ToString(UtilLanguage.ApplyLanguage()["lblcomfrimDelete"])))
                    return;
                bool result = _warehouseBussiness.DeleteWarehouse(this.iDataSource.Copy());
                if (!result)
                {
